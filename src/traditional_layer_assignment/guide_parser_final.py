...
from rc_tree_util import compute_elmore_delays, assign_layers_with_ndla_congestion, direct_rc_tree, compute_net_overflow, update_grid_usage, update_grid_usage_rip_up, update_grid_usage_2d
import sys
...
def main():
    ...
    output_guide_file = os.path.join(folder, "congestion.guide")
    ...
    for net_name, net_2d in net_dict_2d.items():
        iters = 0
        if net_2d.edges_2d != []:
            net2d_with_layers = assign_layers_with_ndla_congestion(net_2d, grid, num_layers=num_layers, two_pass=True, wV=1.0,wC = 2)
            ...
            while (total_overflow_r > total_overflow_og or max_overflow_r > max_overflow_og/3) and iters  < max_iters +1: #3 because 3 for H and 3 for V
                ...
                net2d_with_layers = assign_layers_with_ndla_congestion(net_2d, grid, is_congestion_penalty = True, num_layers=num_layers, two_pass=True, wV=1.0,wC = 2)
                update_grid_usage(net2d_with_layers,grid)
                max_overflow_r, total_overflow_r = compute_net_overflow(net2d_with_layers, grid)
                iters += 1
