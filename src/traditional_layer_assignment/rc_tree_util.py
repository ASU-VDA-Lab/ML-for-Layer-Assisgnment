...
def assign_layers_with_ndla_congestion(net2d, grid=None, is_congestion_penalty=False,num_layers=7,
                                        two_pass=True,wV=1.0,wC=0.6,clamp_k=3.0,cong_power = 2.0):
    src = None
    all_nodes = set(ed.node1 for ed in net2d.edges_2d).union(set(ed.node2 for ed in net2d.edges_2d))
    for n in all_nodes:
        if n.node_type == 'source':
            ...
            if node.parent_edge:
                        ...
                        for (x, y) in cells:
                            ...
                            if capacity > 0: 
                                ...
                                if u > 1.0:
                                    phi += (u - 1.0) ** cong_power
                    if not normalize:
                        normalizer.observe(0.0, via_count, phi)
                        total_cost = via_count
                    else:
                        V_hat = normalizer.norm_V(via_count)
                        C_hat = normalizer.norm_C(phi if is_congestion_penalty else 0.0)
                        total_cost = wV * V_hat + wC * C_hat
                ...
                for layer in default_layers:
                    ...
                    if lf_root is not None and lf_root != layer:
                        ...
                        via_count += initial_stack
                    if not normalize:
                        normalizer.observe(0.0, via_count, 0.0)
                        cost = via_count
                    else:
                        V_hat = normalizer.norm_V(via_count)
                        C_hat = normalizer.norm_C(0.0)
                        cost = wV * V_hat + wC * C_hat
                    dp_table[node][layer] = {
                        "via_count": via_count,
                        "cong": 0.0,
                        "cost": cost,
